...
void BinaryTree<ItemType>::_printInnerNodes(void visit(const ItemType &), BinaryNode<ItemType>* nodePtr) const
{
    if (nodePtr != nullptr)
    {
        _printInnerNodes(visit, nodePtr->getLeftPtr()); // Traverse left subtree
        
        // Check if the current node is an inner node
        if (nodePtr->getLeftPtr() || nodePtr->getRightPtr())
        {
            visit(nodePtr->getItem()); // Call the visit function to print the item
        }
        
        _printInnerNodes(visit, nodePtr->getRightPtr()); // Traverse right subtree
    }
}
